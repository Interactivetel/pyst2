...
class AGI:
    ...
    def send_command(self, command, *args, **kwargs):
        ...
        try:
            ...
            local_command = command.encode("utf8")
            if not PY3:
                local_command = command

            self.stderr.write(f" ({session_id}) COMMAND: {local_command}")
            self.stdout.write(local_command)
            ...
            if not PY3:
                local_command = command
            self.stderr.write(f" ({session_id}) COMMAND: {local_command}. Ex: {e}")
    ...
    def record_file(
        self, filename, format="gsm", escape_digits="#", timeout=DEFAULT_RECORD, offset=0, beep="beep", silence=0
    ):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute(
            "RECORD FILE", self._quote(filename), format, escape_digits, timeout, offset, beep, ("s=%s" % silence)
        )["result"][0]
        try:
            return chr(int(res))
        except:  # noqa
    ...
    def set_variable(self, name, value, session_id: str = None):
        ...
        self.execute("SET VARIABLE", self._quote(name), self._quote(value), session_id=session_id)

    def get_variable(self, name, session_id: str = None):
        ...
        """
        try:
            result = self.execute("GET VARIABLE", self._quote(name), session_id=session_id)
